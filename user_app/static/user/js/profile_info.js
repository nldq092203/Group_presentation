...
if (full_member) {
  let is_member = full_member.dataset.isMember;
    ...
    fetch(`/api/member?username=${username}`, {
      ...
      .then((data) => {
        ...
        if (myself) {
          ...
          edit_info.innerHTML = btn;
        }
      });
  } else {
    let is_requested = full_member.dataset.isRequested;
  ...
  let btn_join = document.querySelector(".btn-join");
  if (btn_edit) {
  ...
  let submit_edit_form = document.querySelector(".info-form-edit");
  if (submit_edit_form) {
    submit_edit_form.addEventListener("submit", (e) => {
      e.preventDefault();
      let content = {
      ...
      skills.forEach((skill, index) => {
        content.skills.push({
          skill: document.querySelector(`#skill-${index}`).value,
        });
      });

      experiences.forEach((experience, index) => {
        content.experiences.push({
          title: document.querySelector(`#experience-title-${index}`).value,
          agency: document.querySelector(`#experience-agency-${index}`).value,
